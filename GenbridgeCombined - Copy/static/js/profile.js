...
(() => {
    ...
    function hidePwSuccess() {
        const el = $("pwSuccessFlash");
        if (!el) return;
    ...
    function showPwSuccess(msg) {
        const el = $("pwSuccessFlash");
        if (!el) return;
        el.textContent = msg || "Password changed successfully";
        el.style.display = "block";

        // optional: scroll user to see it
    ...
    function submitChangePassword(e) {
        ...
        if (!newPw) {
            ...
            showError("errNewPw", "New password cannot be the same as current password.");
            ok = false;
        ...
        (async () => {
            try {
                ...
                if (!res.ok || !data.ok) {
                    const field = data.field || "current";
                    const msg = data.message || "Failed to update password.";

                    if (field === "current") showError("errCurrentPw", msg);
                    else if (field === "new") showError("errNewPw", msg);
                    else if (field === "confirm") showError("errConfirmPw", msg);
                    else showError("errCurrentPw", msg);
                ...
                $("confirmPw").value = "";
                closeChangePassword();
                showPwSuccess((data && data.message) ? data.message : "Password changed successfully");
            } catch (err) {
                showError("errCurrentPw", "Error updating password. Please try again.");
            }
        })();
    ...
    document.addEventListener("DOMContentLoaded", () => {
        // Auto-hide server-rendered success flashes 
        document.querySelectorAll(".flash.success").forEach((el) => {
            if (el.id === "pwSuccessFlash") return;
