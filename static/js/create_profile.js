...
(() => {
    ...
    function validateStep1() {
        ...
        const minAge = Number(($("dob").dataset.minAge) || 16);
        const errDob = $("err_dob");

        if (!dob) {
            if (errDob) errDob.textContent = "Please enter your date of birth.";
            showErr("err_dob", true);
            ok = false;
        } else if (!isAtLeastAge(dob, minAge)) {
            if (errDob) errDob.textContent = "minimum age requirement is 16";
            showErr("err_dob", true);
            ok = false;
        } else {
            showErr("err_dob", false);
        }
    ...
    function isAtLeastAge(isoDob, minAge) {
    ...
    function wireDobAgeMessage() {
        const dobEl = $("dob");
        const errEl = $("err_dob");
        const minAge = Number((dobEl?.dataset?.minAge) || 16);
        if (!dobEl || !errEl) return;

        // Auto-restrict: prevent selecting a DOB newer than today - 16 years
        const maxDob = maxDobForMinAge(minAge);
        dobEl.setAttribute("max", maxDob);
        ...
        function validateDob() {
            ...
            if (!isAtLeastAge(v, minAge)) {
                errEl.textContent = "minimum age requirement is 16";
                showErr("err_dob", true);
            } else {
                showErr("err_dob", false);
            }
        ...
        // Change catches picker selection; input catches manual typing
        dobEl.addEventListener("change", validateDob);
        dobEl.addEventListener("input", validateDob);
    }
