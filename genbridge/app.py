...
# DB helpers (from your uploaded db.py)
from db import (
    list_events,
    get_event,
...
app.secret_key = "genbridge-secret-key"  # you can change later
...
def enrich_datetime_fields(obj: Dict[str, Any]) -> Dict[str, Any]:
    """
    Adds common display fields your templates might use:
      - date_long: "Mon, 15 Dec 2025"
      - time_only: "14:00"
      - date_fmt:  "Mon, 15 Dec 2025 at 14:00"
    Works for both event dicts and booking dicts (as long as key 'date' exists).
    """
...
def event_details(event_id: int):
    ...
    return render_template("event_details.html", event=ev)
...
def register_form(event_id: int):
    ...
    enrich_datetime_fields(ev)
    return render_template("register_form.html", event=ev)
...
def register_submit(event_id: int):
    ...
    # Some of your forms include consent checkbox (safe to support)
    ...
    # If consent checkbox exists in your form, require it
    ...
    # Decrement spots first (atomic-ish)
...
def edit_booking_post(booking_id: int):
    ...
    # IMPORTANT: db.py update_booking uses keyword-only fields after booking_id
    ok = update_booking(
        booking_id,
        first_name=first_name,
        last_name=last_name,
...
# IMPORTANT: your bookings.html calls url_for('clear_bookings')
